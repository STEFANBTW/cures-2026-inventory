...
const{jsPDF}=window.jspdf;const originalText=pdfBtn.querySelector('h1').innerText;pdfBtn.querySelector('h1').innerText="Generating PDF...";try{const doc=new jsPDF();const pageWidth=doc.internal.pageSize.width;const pageHeight=doc.internal.pageSize.height;const margin=15;let yPos=20;const grandTotal=inventory.reduce((sum,i)=>sum+((i.basePrice+(i.shipping||0))*(i.quantity||1)),0);const totalBasePrice=inventory.reduce((sum,i)=>sum+(i.basePrice*(i.quantity||1)),0);const totalShipping=inventory.reduce((sum,i)=>sum+((i.shipping||0)*(i.quantity||1)),0);const categories={};inventory.forEach(item=>{if(!categories[item.category])categories[item.category]=[];categories[item.category].push(item);});doc.setFont("helvetica","bold");doc.setFontSize(16);doc.setTextColor(40,40,40);doc.text("CURE PROSPECTS 2025-2026",margin,yPos);doc.setFontSize(18);doc.setTextColor(0,0,0);doc.text(getFormattedPrice(grandTotal),pageWidth-margin,yPos,{align:"right"});yPos+=6;doc.setFont("helvetica","normal");doc.setFontSize(9);doc.setTextColor(100,100,100);const totalBreakdownText=`Products:${getFormattedPrice(totalBasePrice)}+Shipping:${getFormattedPrice(totalShipping)}`;doc.text(totalBreakdownText,pageWidth-margin,yPos,{align:"right"});yPos+=5;doc.setDrawColor(200,200,200);doc.setLineWidth(0.5);doc.line(margin,yPos,pageWidth-margin,yPos);yPos+=15;const categoryOrder=['Learning','Repair','Productivity'];categoryOrder.forEach(catName=>{if(!categories[catName])return;const items=categories[catName];if(yPos>pageHeight-30){doc.addPage();yPos=20;}
doc.setFont("helvetica","bold");doc.setFontSize(14);doc.setTextColor(0,51,102);doc.text(catName.toUpperCase(),margin,yPos);yPos+=10;items.forEach(item=>{const quantity=item.quantity||1;const unitTotal=parsePrice(item.basePrice)+parsePrice(item.shipping||0);const itemTotal=unitTotal*quantity;const shippingText=parsePrice(item.shipping)===0?"Free Shipping":`Shipping:${getFormattedPrice(parsePrice(item.shipping||0))}`;let breakdownText=`(Base:${getFormattedPrice(parsePrice(item.basePrice))})`;if(quantity>1){breakdownText=`(${quantity}@${getFormattedPrice(unitTotal)}=${getFormattedPrice(itemTotal)})`;}
const rowHeight=25;if(yPos+rowHeight>pageHeight-15){doc.addPage();yPos=20;}
doc.setFillColor(240,240,240);doc.rect(margin,yPos,15,15,'F');doc.setFontSize(6);doc.setTextColor(150,150,150);doc.text("IMG",margin+4,yPos+9);const unitBase=parsePrice(item.basePrice);const unitShip=parsePrice(item.shipping||0);const totalCost=(unitBase+unitShip)*quantity;doc.setFont("helvetica","bold");doc.setFontSize(12);doc.setTextColor(0,0,0);let nameLines=doc.splitTextToSize(item.nickname||item.name,maxNameWidth);if(nameLines.length>2){let secondLine=nameLines[1];if(secondLine.length>3){secondLine=secondLine.substring(0,secondLine.length-3)+"...";}
nameLines=[nameLines[0],secondLine];}
doc.text(nameLines,textStartX,yPos+5);const detailYOffset=nameLines.length>1?11+5:11;doc.setFont("helvetica","normal");doc.setFontSize(10);doc.setTextColor(80,80,80);doc.text(`${item.productStore}|${item.source}|${shippingText}`,textStartX,yPos+detailYOffset);doc.setFont("helvetica","bold");doc.setFontSize(12);doc.setTextColor(0,0,0);doc.text(getFormattedPrice(itemTotal),pageWidth-margin,yPos+5,{align:"right"});doc.setFont("helvetica","italic");doc.setFontSize(8);doc.setTextColor(120,120,120);doc.text(breakdownText,pageWidth-margin,yPos+11,{align:"right"});yPos+=rowHeight;doc.setDrawColor(230,230,230);doc.line(margin,yPos-8,pageWidth-margin,yPos-8);});yPos+=5;});doc.save('CURE_Prospects_2025-2026_Inventory.pdf');}catch(err){console.error(err);alert("Error generating PDF. See console for details.");}finally{pdfBtn.querySelector('h1').innerText=originalText;}});}
