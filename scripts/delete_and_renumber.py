...
# IDs to delete (from user): 8,10,28,29,34,37,40,42,43,50,53,55,66,70
IDS_TO_DELETE = {8,10,28,29,34,37,40,42,43,50,53,55,66,70}
...
def main():
    ...
    for it in items:
        try:
            ...
            iid = -1
        if iid in IDS_TO_DELETE:
            removed.append(iid)
        else:
            filtered.append(it)

    # renumber sequentially starting at 1, preserving order
    for i, it in enumerate(filtered, start=1):
        it['id'] = i

    new_count = len(filtered)

    new_array_text = json.dumps(filtered, indent=4, ensure_ascii=False)
    new_js = 'const inventory = ' + new_array_text + ';\n\nmodule.exports = { inventory };\n'

    main_path.write_text(new_js, encoding='utf-8')
    print(f"Removed {len(removed)} items: {sorted(removed)}. Renumbered to {new_count} items (was {orig_count}).")
