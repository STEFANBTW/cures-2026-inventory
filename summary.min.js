...
let summaryCardObserver=null;function initSummaryAnimations(){const cards=document.querySelectorAll('.summary-card-wrapper');if(cards.length===0)return;if(summaryCardObserver){summaryCardObserver.disconnect();}
const observerOptions={root:null,rootMargin:'0px',threshold:0.15};summaryCardObserver=new IntersectionObserver((entries,obs)=>{entries.forEach(entry=>{if(entry.isIntersecting){const delay=entry.target.dataset.delay||0;setTimeout(()=>{entry.target.classList.add('in-view');},delay);obs.unobserve(entry.target);}});},observerOptions);cards.forEach((card,index)=>{let staggerTime=(index%5)*100;card.dataset.delay=staggerTime;summaryCardObserver.observe(card);setTimeout(()=>{if(!card.classList.contains('in-view'))card.classList.add('in-view');},1000+staggerTime);});}